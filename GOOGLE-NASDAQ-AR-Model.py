...
data = pd.read_csv('GOOGLE - NASDAQ.csv', parse_dates=['Date'], index_col='Date', date_parser=dateparse)
...
expweighted_avg = ts_log.ewm(halflife=7,min_periods=0,adjust=True,ignore_na=False).mean()
plt.plot(ts_log)
...
#AR model
model = ARIMA(ts_log_ewma_diff, order=(18, 0, 0))
...
predictions_AR_next_log = model.predict(results_AR.params, start=1969, end=1980)
